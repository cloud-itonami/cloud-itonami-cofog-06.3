(ns leaksurvey.operation
  "SurveyOperationActor -- one leak-detection survey request = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Survey Advisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Water Infrastructure Governor
  ...
  each operation is auditable and checkpointed. An inspector's survey
  lifecycle is advanced by MANY ops (pipe-condition logging /
  leak-survey filing / repair-recommendation scheduling / critical-leak
  escalation), each independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human survey coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [leaksurvey.advisor :as advisor]
            [leaksurvey.governor :as governor]
...
(defn build
  ...
  opts:
    :advisor      -- a `leaksurvey.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Survey Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Water Infrastructure Governor -- independent censor.
      (g/add-node :govern
